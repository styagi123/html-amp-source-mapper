...
const disposable = vscode.commands.registerCommand('ampMapper.togglePreview', () => {
    ...
    const changeSelection = vscode.window.onDidChangeTextEditorSelection(e => {
        if (e.textEditor !== editor || !currentPanel) return;
...
function processAmp(code: string): string {
    ...
    return lines.map((line, index) => {
        ...
        if (/<\s*td/i.test(line)) tag = 'td';
        else if (/<\s*th/i.test(line)) tag = 'th';
        else if (/<\s*tr/i.test(line)) tag = 'tr';
        else if (/<\s*table/i.test(line)) tag = 'table';
        else if (/<\s*div/i.test(line)) tag = 'div';
        else if (/<\s*p/i.test(line)) tag = 'p';
        else if (/<\s*h[1-6]/i.test(line)) tag = 'heading';
        else if (/<\s*button/i.test(line)) tag = 'button';
        else if (/<\s*span/i.test(line)) tag = 'span';
...
// 🔥 Update Preview
function updatePreview(panel: vscode.WebviewPanel, code: string) {
